...
export function lerConfig() {
  try {
    ...
    return { codigo: novoCodigoSala(), ligado: false };
  }
}

export function guardarConfig(config) {
  const limpo = {
    codigo: config.codigo || novoCodigoSala(),
...
export function urlDoOverlay(config) {
  ...
  const p = new URLSearchParams({ codigo: config.codigo || 'mesa' });
...
/**
 * Manda o estado atual para o overlay via WebRTC P2P.
 */
export async function publicar(config, estado) {
  ...
  return { ok: true, espectadores: hostP2P.conexoes.size };
}

// ---------------------------------------------------------------- subscrever

/**
 * Ouve as atualizações no Overlay/Mestre via WebRTC P2P. Devolve uma função para desligar.
